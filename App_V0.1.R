...
#install.packages(c("shiny", "sf", "ggplot2", "viridis", "dplyr", "readr", "ggspatial", "countrycode", "bslib","leaflet","RColorBrewer","shinythemes","mapview"))
...
library(mapview)       # For mapshot function if needed
...
server <- function(input, output, session) {
  ...
  output$dataMap <- renderLeaflet({
    ...
    if (!is.null(malaria_data()) && input$metric %in% malaria_data()$metric) {
      ...
      merged_data <- merge(spatial_data, data, by.x = "shapeName", by.y = "shapeName", all.x = TRUE)
    }
